...
namespace NAL_SMTA
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            ConDB.GetShift();
            tBPW.PasswordChar = '*';
            dGVMakerLot.Rows.Clear();

            var appSettings = ConfigurationManager.AppSettings;
            string vComport = appSettings.Get("COMPORT");
            string vBaudRate = appSettings.Get("BaudRate");

            //
            // Set RS-232C parameters.
            //
            this.serialPortKN.BaudRate = int.Parse(vBaudRate);         // 9600, 19200, 38400, 57600 or 115200
            this.serialPortKN.DataBits = 8;              // 7 or 8
            this.serialPortKN.Parity = Parity.Even;    // Even or Odd
            this.serialPortKN.StopBits = StopBits.One;   // One or Two
            this.serialPortKN.PortName = vComport;

            serialPortInstance = new SerialPort[SERIAL_PORT_COUNT] { this.serialPortKN };
        }
        private void MainFrom_Load(object sender, EventArgs e)
        {
            ...
            //if (customer == "Transtron" & Process == "Final")
            //{
            //    btPrtTag.Show();
            //    //btPrintTagNew.Show();
            //    btExportFinal.Show();
        ...
        private void tBMatQr_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                //tBLotNo.Enabled = false;
                ...
                string QrTag = tBMatQr.Text;
                try
                {
                    string[] vDateSAP = QrTag.Split('@');
                    vBatch = vDateSAP[2];
                    vItem = vDateSAP[3];

                    string[] vMatData = ConDB.GetMakerLot(vBatch);

                    if (vMatData.Length > 3)
                    {
                        sMaterial_No = vMatData[0];
                        sMatDesc = vMatData[1];
                        sMakerLot = vMatData[2];
                        sMakerLot2 = vMatData[3];



                        //tBMatQr.Text = vBatch;// sMakerLot;
                        //tBMakerLot.Text = sMakerLot;//sMatDesc;

                        //string[] vItemCoat = { "1248-04345", "1248-04346","8001-00515","8001-00516"};
                        //string[] vItemSolder = { "1248-04340" };
                        //string[] vItemAssy = { "1418-00308", "1418-00474", "1418-00508" };

                        string vResult = ConDB.PSES_CHK_MAKER_ITEM(Process, vItem);

                        if (Process == "Soldering" && vResult == "1")
                        {
                            DataGridViewRow row = new DataGridViewRow();
                            row.CreateCells(dGVMakerLot);  // this line was missing
                            row.Cells[0].Value = vBatch;
                            row.Cells[1].Value = sMatDesc;
                            row.Cells[2].Value = sMakerLot;
                            row.Cells[3].Value = sMaterial_No;

                            int numRows = dGVMakerLot.Rows.Count;

                            if (numRows == 0)
                            {
                                dGVMakerLot.Rows.Add(row);
                                dGVMakerLot.Refresh();
                                numRows = dGVMakerLot.Rows.Count;
                                if ((Process == "Soldering" || Process == "ASSY") && numRows == 1)
                                {
                                    string[] vPointQty = vDateSAP[4].Trim().Split('.');
                                    //Qty = int.Parse(vPointQty[0]);
                                ...
                                else if (Process == "COAT" && numRows == 2)
                                {
                                    string[] vPointQty = vDateSAP[4].Trim().Split('.');
                                    //Qty = int.Parse(vPointQty[0]);
                            ...
                            {
                                string vMakerLot1 = dGVMakerLot.Rows[0].Cells[2].Value.ToString();

                                if (vMakerLot1 != sMakerLot)
                                {
                                    dGVMakerLot.Rows.Add(row);
                                    dGVMakerLot.Refresh();
                                    numRows = dGVMakerLot.Rows.Count;

                                    if (Process == "Soldering" && numRows == 1)
                                    {
                                        string[] vPointQty = vDateSAP[4].Trim().Split('.');
                                        //Qty = int.Parse(vPointQty[0]);
                                ...
                                {
                                    MsgFroms("Duplicate Maker Lot.", "Please SAP label.", 1);
                                    tBMatQr.Clear();
                                    tBMatQr.Focus();
                                }
                            }
                        }
                        else if (Process == "COAT" && vResult == "1")
                        {
                            DataGridViewRow row = new DataGridViewRow();
                            row.CreateCells(dGVMakerLot);  // this line was missing
                            row.Cells[0].Value = vBatch;
                        ...
                        else if (Process == "ASSY" && vResult == "1")
                        {
                            DataGridViewRow row = new DataGridViewRow();
                            row.CreateCells(dGVMakerLot);  // this line was missing
                            row.Cells[0].Value = vBatch;
                        ...
                        {
                            MsgFroms("Please check SAP Label.", "", 1);
                            tBMatQr.Clear();
                            tBMatQr.Focus();
                        }
                    }
                    ...
                    {
                        tBMatQr.Clear();
                        MsgFroms("Not Found Maker Lot in system.", "Please call TC 216,230", 1);
                        ShowStatusNG();
                    }
                }
                catch
        ...
        private void UpdateFontdGV()
        {
            ...
            dGVMakerLot.ColumnHeadersDefaultCellStyle.Font = new Font("Tahoma", 8, FontStyle.Bold);
            dGVMakerLot.DefaultCellStyle.Font = new Font("Tahoma", 8);
        }
        private void InsDataBarcode(string Barcode, string Seiban)
        {
            ...
            try
            {
                ...
                if (result[1] == "1")
                {
                    if(Process == "ASSY")
                    {
                        InsDataBarcodeKaker(Cover);
                    }
                    else
                    {
                        InsDataBarcodeKaker(Barcode);
                    }
        ...
        private void InsDataBarcodeKaker(string Barcode)
        {
            var appSettings = ConfigurationManager.AppSettings;
            ...
            try
            {
                ...
                if (numRows > 0)
                {
                    ...
                    {
                        Batch = dGVMakerLot.Rows[i].Cells[0].Value.ToString();
                        ItemDESC = dGVMakerLot.Rows[i].Cells[1].Value.ToString();
                        MakerLot = dGVMakerLot.Rows[i].Cells[2].Value.ToString();
                        ItemCD = dGVMakerLot.Rows[i].Cells[3].Value.ToString();

                        result = ConDB.STBL_IND_MAKERLOT(Process, Barcode, ItemCD, ItemDESC, Batch, MakerLot, Emp, CLIENT, Shift);
                    }
                }
            }
            catch (Exception e)
        ...
        public void ShowStatus()
        {
            ...
            lBMainSts.Font = new Font("Tahoma", 50, FontStyle.Bold);
        }
        public void ShowStatusOK()
        {
            lBMainSts.BackColor = Color.Aquamarine;
            lBMainSts.Text = "Success";
        ...
        private void btNewMakerLot_Click(object sender, EventArgs e)
        {
            dGVMakerLot.Rows.Clear();
            tBMatQr.Clear();
            tBMatQr.Enabled = true;
        ...
        public void Connect_Comport()
        {
            for (int i = 0; i < serialPortInstance.Length; i++)
            {
                try
                {
                    ...
                    lBScanSTS.Text = "Connected";
                }
                catch (Exception ex)
                {
                    lBScanSTS.Text = "Not Found.";
        ...
        public void TimingON()
        {
            ...
            Byte[] sendBytes = System.Text.ASCIIEncoding.ASCII.GetBytes(lon);
            ...
            for (int i = 0; i < serialPortInstance.Length; i++)
            {
                if (this.serialPortInstance[i].IsOpen)
                {
                    ...
                    catch (IOException ex)
                    {
                        ...
                        lBScanSTS.Text = "Error.";
                    }
                }
                else
                {
                    //MessageBox.Show(serialPortInstance[i].PortName + " is disconnected.");
                    lBScanSTS.Text = "Disconnected.";
                }
            }
        }
        public void ReadDataComport()
        {
            ...
            for (int i = 0; i < this.serialPortInstance.Length; i++)
            {
                ...
                {
                    lBScanSTS.Text = " is disconnected.";
                    continue;
                }
                ...
                for (; ; )
                {
                    ...
                    {
                        lBScanSTS.Text = ex.Message;    // disappeared
                        break;
                    }
                    if (recvSize == 0)
                    {
                        lBScanSTS.Text = " has no data.";
                        break;
                    }
                    if (recvBytes[0] == STX)
                    {
                    ...
                    else
                    {
                        ...
                        string Serial = Encoding.GetEncoding("Shift_JIS").GetString(recvBytes);

                        tBBarcode.Paste(Serial.Substring(0, 14));
        ...
        private void serialPortKN_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
